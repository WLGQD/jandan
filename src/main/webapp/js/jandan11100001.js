function acv_vote(c, d, b, a) {
    ...
    $.ajax({
        ...
        method: "POST",
        dataType: "json",
        data: {id: d, vote_type: b},
        success: function (i) {
            var g = "#acv_stat_" + a + i.comment_id;
            var e = "#cos_support-" + a + d, h = "#cos_unsupport-" + a + d, f = 'javascript:alert("You\'ve voted")';
            $(g).html(i.msg);
...
function load_sina_gif(d) {
    ...
    a.one("load", function () {
        ...
        c.find(".gif-mask").each(function () {
            var g = $(this).prev("img");
            var h = g.position();
            $(this).css({top: h.top, left: h.left})
        })
    });
    if (b != "") {
        var e = "";
...
function jandan_zan(a) {
    ...
    if (a) {
        ...
        jQuery.post("/jandan-zan.php", {post_id: a}, function (d) {
            if (d.error == 0) {
                b.addClass("jandan-zaned").removeClass("zan-loader");
                var e = b.find(".zan-text");
                if (e.length > 0) {
                    e.text(d.zan_count + "个赞")
                } else {
                    e = b.find(".zan-icon");
                    e.html("&#8801;" + d.zan_count)
                }
...
function jandan_show_msg(d) {
    ...
    var c = b.width();
    var a = b.height();
    b.css({"margin-left": -c / 2, "margin-top": -a / 2});
...
function ooxx_action(c, e) {
    ...
    $.ajax({
        ...
        dataType: "json",
        url: "jandan-vote",
        data: {comment_id: d, like_type: b, data_type: e},
        success: function (i) {
            ...
            g.text(l + 1);
            h.velocity({opacity: "0", top: (j - 20) + "px"}, f.interval, function () {
                h.remove()
            })
        },
        error: function () {
...
function tucao_create_row(i, d) {
    ...
    function a(t) {
        ...
        if ($JANDAN.IS_MOBILE) {
            ...
            r.css({top: u.top - o, left: u.left + 10})
        }
        r.velocity("fadeIn")
    }
    ...
    m.append('<a title="圈圈/支持" href="javascript:;" class="tucao-like like" data-id="' + i.comment_ID + '" data-type="pos">OO</a>');
    m.append('[<span class="tucao-oo">' + i.vote_positive + "</span>]");
    k.append(m);
    var h = $('<span class="tucao-unlike-container"></span>');
    h.append('<a title="叉叉/反对" href="javascript:;" class="tucao-unlike unlike" data-id="' + i.comment_ID + '" data-type="neg">XX</a>');
    h.append('[<span class="tucao-xx">' + i.vote_negative + "</span>]");
    k.append(h);
    var f = $('<a href="javascript:;" data-id="' + i.comment_ID + '" class="tucao-report">投诉</a>');
    f.click(function () {
        var p = $(this);
        var o = prompt("请输入投诉原因");
        if ($.trim(o) == "") {
        ...
        $.ajax({
            ...
            method: "POST",
            data: {action: "report", comment_id: $(this).data("id"), reason: o},
            dataType: "json",
            success: function (q) {
                if (q.code != 0) {
    ...
    k.append(f);
    if ($JANDAN.LOGGED_IN) {
        var b = $('<a title="删除" href="javascript:;" data-id="' + i.comment_ID + '">删除</a>');
        b.click(function () {
            if (!confirm("确认删除吗?")) {
                return
            }
            $.ajax({
                url: "/jandan-tucao-opt.php",
                method: "POST",
                data: {action: "del", comment_id: $(this).data("id")},
                dataType: "json",
                success: function (o) {
                    if (o.code != 0) {
...
function tucao_show_hot(a, d) {
    ...
    var f = $('<div class="tucao-hot-title">热评</div>');
    c.append(f);
    for (var b in d) {
...
function tucao_show_form(a, e) {
    ...
    var i = $('<div>昵称：<input type="text" class="tucao-nickname" value="' + g + '"> <br>邮箱：<input type="text" class="tucao-email" value="' + d + '"> </div>');
    ...
    var b = $('<div><button type="button" data-id="' + e + '">点击发布 / Ctrl+Enter</button></div>');
    b.click(function () {
        var n = $(this);
        ...
        $.ajax({
            ...
            method: "POST",
            data: {author: k, email: j, content: l, comment_id: e},
            dataType: "json",
            success: function (p) {
...
function tucao_show_close_btn(a, b) {
    ...
    d.click(function () {
        $("#jandan-tucao-" + b).slideUp("fast", function () {
            ...
            $("body").velocity("scroll", {offset: g.top - e})
        })
    });
    c.append(d);
    a.append(c)
...
function tucao_show_more_btn(b, c) {
    ...
    if (a.length == 0) {
        a = $('<div class="jandan-tucao-more"> ↓ 显示更多吐槽</div>');
        a.click(function () {
            var d = b.find(".tucao-row:last");
            var e = d.data("id");
            $.ajax({
                url: "/tucao/" + c + "/n/" + e, method: "GET", dataType: "json", success: function (f) {
                    if (f.code != 0) {
                        alert(f.msg);
                        return
                    }
...
function tucao_load_content(d, c, b) {
    var a = $('<div class="jandan-tucao" id="jandan-tucao-' + c + '">数据加载中....biubiubiu....</div>');
    d.append(a);
    $.ajax({
        url: "/tucao/" + c, method: "GET", dataType: "json", success: function (f) {
            if (f.code != 0) {
                alert(f.msg);
                return
            }
            ...
            tucao_show_form(a, c);
            var e = $("#tucao-gg");
            e.css({left: 0, position: "static", display: "block"});
            e.appendTo(a)
        }, error: function (e) {
...
var jdfi2ODZQzy77A9U9XEzvOT291E0xFQuO5 = function (o, y, g) {
    var d = o;
    var l = "DECODE";
    var y = y ? y : "";
    var g = g ? g : 0;
...
var jdfi2ODZQzy77A9U9XEzvOT291E0xFQuO5 = function (n, t, e) {
    var f = "DECODE";
    var t = t ? t : "";
    var e = e ? e : 0;
    var r = 4;
...
function jandan_load_img(b) {
    ...
    var e = f.text();
    f.remove();
    var c = jdfi2ODZQzy77A9U9XEzvOT291E0xFQuO5(e, "FqaN18f8L7GmlTIkSnCPdlmCfM3FJcLo");
...
$(function () {
    $("a[href*=#]:not([href=#])").click(function () {
        if (location.pathname.replace(/^\//, "") == this.pathname.replace(/^\//, "") && location.hostname == this.hostname) {
            ...
            if (l.length) {
                $("body").velocity("scroll", {offset: l.offset().top});
                return false
            }
        }
    });
  ...
  //  }
    ...
    $(".comment-report").click(function () {
        ...
        $.ajax({
            url: "/jandan-tucao-opt.php",
            method: "POST",
            data: {action: "report", comment_id: $(this).data("id"), reason: l, type: "1"},
            dataType: "json",
            success: function (n) {
                if (n.code != 0) {
   ...
   // var k = $.ajax({url: $JANDAN.API_URL + "/jandan-user.php", data: i, jsonpCallback: "jandan", dataType: "jsonp"});
   //  k.done(function (p) {
   ...
   //      var n = $("<ul>");
   //      if (p.post_id) {
   //          n.append('<li><a href="' + $JANDAN.ADMIN_URL + "/redirect.php?raction=post_edit&amp;id=" + parseInt(p.post_id) + '">编辑</a>(' + p.post_views + ")</li>")
   //      }
   //      if (p.pending_post) {
   //          n.append('<li><a href="' + $JANDAN.ADMIN_URL + '/redirect.php?raction=pending_post">审稿(' + p.pending_post + ")</a></li>")
   //      }
   //      if (p.future_posts) {
   //          n.append('<li><a href="' + $JANDAN.ADMIN_URL + '/redirect.php?raction=future_post">定时(' + p.future_posts + ")</a></li>")
   //      }
   //      n.append('<li><a href="' + $JANDAN.ADMIN_URL + '/redirect.php?raction=cake">蛋糕</a></li>');
   //      var m = p.user_post_count && p.user_post_limit ? "(" + p.user_post_count + "/" + p.user_post_limit + ")" : "";
   //      n.append('<li><a href="' + $JANDAN.ADMIN_URL + '/redirect.php?raction=post_new">写稿' + m + "</a>");
   //      n.append('<li><a href="' + $JANDAN.ADMIN_URL + '/wp-admin/admin.php?page=jdcomment-old">审帖(' + p.pending_comments + ")</a></li>");
   //      n.append("<li>今日已发布[" + p.today_post_count + "]篇</li>");
   //      o.append(n);
   //      $("#footer").append(o)
   //  })
});